...
from .normalization import clean_text, norm_key
...
def unique_album_artist(records: list[TrackRecord]) -> list[AlbumArtistRecord]:
    ...
    for rec in records:
        ...
        key = (norm_key(artist), norm_key(album))
        if key in seen:
            continue
        seen.add(key)
        unique.append(AlbumArtistRecord(artist=artist, album=album))

    unique.sort(key=lambda r: (norm_key(r.artist), norm_key(r.album)))
    return unique
...
def spotify_not_owned(local_tracks: list[TrackRecord], spotify_tracks: list[TrackRecord]) -> list[AlbumArtistRecord]:
    ...
    owned_keys = {(norm_key(r.artist), norm_key(r.album)) for r in local_pairs}

    missing = [
        rec
        for rec in spotify_pairs
        if (norm_key(rec.artist), norm_key(rec.album)) not in owned_keys
    ]

    missing.sort(key=lambda r: (norm_key(r.artist), norm_key(r.album)))
    return missing
